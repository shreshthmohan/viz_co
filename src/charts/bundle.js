(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.mace = {}, global.d3));
  ...
  const formatNumber = function (
    ...
    return scientificNotations
      ? formattedValue
      : lodash.replace(formattedValue, 'G', 'B')
  };
...
`);
    ...
    const nameValues = lodash(data).map(nameField).uniq().value();
    ...
    cGroup
      .on('mouseover', (e, d) => {
        ...
        <br />
        ${lodash.capitalize(sizeField)}: ${sizeFieldValue}
        `,
        );
        tooltipDiv
    ...
    goToInitialState.on('click', () => {
      ...
      lodash.forEach(defaultStateAll, val => {
  ...
  const validateData = ({ data, dimensionTypes, dimensions }) => {
    ...
    lodash.forEach(dimensionTypes, (valFuncs, dim) => {
      lodash.forEach(valFuncs, valFunc => {
    ...
    if (lodash.isEmpty(dataValidations)) return { valid: true, message: '' }
    let messageReadable = 'Data Validation Errors </br>';
    lodash.mapKeys(dataValidations, (val, key) => {
      const msgDetails = lodash.map(
        val,
        val_ =>
          `</br>&nbsp;&nbsp;&nbsp;${val_.validation} : row numbers - ${lodash.map(
          val_.message,
          'rowNumber',
        ).join(', ')} | values - ${lodash.map(val_.message, key).join(', ')}`,
      ).join('');
  ...
  const shouldBeNumber = ({ data, dimensions, dim }) => {
    const invalidRows = lodash.filter(data, (row, i) => {
      // eslint-disable-next-line no-param-reassign
      row.rowNumber = i + 1;
      return Number.isNaN(Number(row[dimensions[dim]]))
    });

    if (lodash.isEmpty(invalidRows)) return { valid: true, message: '' }
  ...
  const shouldBeUnique = ({ data, dimensions, dim }) => {
    ...
    const duplicates = lodash(data)
      .countBy(dimensions[dim])
      .pickBy(val => val > 1)
      .keys()
      .value();

    const duplicateRows = lodash.filter(data, (row, i) => {
      // eslint-disable-next-line no-param-reassign
      row.rowNumber = i + 1;
      return lodash.includes(duplicates, row[dimensions[dim]])
    });

    if (lodash.isEmpty(duplicates)) return { valid: true, message: '' }
  ...
  const shouldNotBeBlank = ({ data, dimensions, dim }) => {
    const invalidRows = lodash.filter(data, (row, i) => {
      // eslint-disable-next-line no-param-reassign
      row.rowNumber = i + 1;
      return lodash.isEmpty(row[dimensions[dim]])
    });
  ...
  const optionValidation = ({ optionTypes, options }) => {
    ...
    lodash.each(optionTypes, (fn, key) => {
  ...
  const checkColor = val => {
    ...
    lodash.forEach(parsedColor, function (value) {
      result.valid = result.valid && !lodash.isNaN(value);
    });
  ...
  function validateColumnsWithDimensions({ columns, dimensions }) {
    ...
    if (lodash.isEmpty(dimensions)) {
      return { valid: false, message: 'No dimensions provided.' }
    }
    const parsedDimensions = {};
    lodash.each(dimensions, (val, key) => {
    ...
    lodash.each(parsedDimensions, (val, key) => {
      const exists = lodash.includes(columns, val);
  ...
  function showErrors(svgParentNodeSelector, errorMessages) {
    ...
    if (lodash.isEmpty(errorMessages)) {
      return
    }
